...
bool WorldSystem::step(float elapsed_ms_since_last_update) {
	...
	if (registry.hardShells.components.size() <= MAX_TURTLES && next_turtle_spawn < 0.f) {
		// Reset timer
		next_turtle_spawn = (TURTLE_DELAY_MS / 3) + uniform_dist(rng) * (TURTLE_DELAY_MS / 2);
		// Create turtle
		Entity entity = createTurtle(renderer, {0,0});
		// Setting random initial position and constant velocity
		Motion& motion = registry.motions.get(entity);
		motion.position =
			vec2(enemy_m.position.x, enemy_m.position.y + enemy_m.scale.y / 2);
		// reduce speed by /2
	...
	if (registry.softShells.components.size() <= MAX_FISH && next_fish_spawn < 0.f) {
		...
		Motion& motion = registry.motions.get(entity);
		motion.position =
			vec2(50.f + uniform_dist(rng) * (screen_width - 100.f), 200.f);
		// reduce speed by /2
		motion.velocity = vec2( 0.f, 300.f);
	}
...
void WorldSystem::restart_game() {
	...
	// Create a new salmon
	player_salmon = createSalmon(renderer, { 200, 700 });
	registry.colors.insert(player_salmon, {1, 0.8f, 0.8f});
	enemy = creatEnemy(renderer, {600, 100});
	block = createBlock(renderer, {600, 700});
...
void WorldSystem::on_key(int key, int, int action, int mod) {
	...
	if (key == GLFW_KEY_LEFT) {
		if (action == GLFW_PRESS || action == GLFW_REPEAT) {
			registry.motions.get(player_salmon).velocity.x = -300;
		}
		if (action == GLFW_RELEASE) {
			registry.motions.get(player_salmon).velocity.x = 0;
		}
	}
